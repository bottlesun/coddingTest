 ...
 * 즉 프로그램 시작 이후 초기에 k일 까지는 모든 출연 가수의 점수가 명예의 전당에 오르게 됩니다.
 ...
 *
 * 명예의 전당 목록의 점수의 개수 k, 1일부터 마지막 날까지 출연한 가수들의 점수인 score 가 주어졌을 때,
 * 매일 발표된 명예의 전당의 최하위 점수를 return 하는 solution 함수를 완성해주세요.
...
const solution = (k, score) => {
  ...
  for(let i = 0; i < score.length; i++ ){
    if( i < k) {
      halloOfFame.push(score[i])
    } else {
      halloOfFame.push(score[i])
      halloOfFame.sort((a,b) => a-b).shift()
    }
    answer.push(halloOfFame[0])
  }
  console.log(answer)
  return answer
}
